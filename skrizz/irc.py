...
class Bot(asynchat.async_chat):
    def __init__(self, config):
        ...
        """Skrizz's current ``Nick``. Changing this while Skrizz is running is
        untested."""
        self.user = config.user
        """Skrizz's user/ident."""
        self.name = config.name
        """Skrizz's "real name", as used for whois."""

        self.verbose = True
        """True if Skrizz is running in verbose mode."""
        self.channels = []
        """The list of channels Skrizz is currently in."""
    ...
    def log_raw(self, line, prefix):
        ...
        if not os.path.isdir(self.config.core.logdir):
            try:
                ...
                stderr('Please fix this and then run Skrizz again.')
                os._exit(1)
    ...
    def write(self, args, text=None):
        ...
        In other words, both ``skrizz.write(('PRIVMSG',), 'Hello, world!')``
        and ``skrizz.write(('PRIVMSG', ':Hello, world!'))`` will send
