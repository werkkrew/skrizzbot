...
The config class is an abstraction class for accessing the active Skrizz
configuration file.

The Skrizz config file is divided to sections, and each section contains keys
...
the Skrizz core. Modules are allowed to read those, but must not change them.
...
through creating settings for the Skrizz core, the settings database, and any
...
"""
"""
Config - A config class and writing/updating utility for Skrizz
...
class Config(object):
    def __init__(self, filename, load=True, ignore_errors=False):
        ...
        if load:
            ...
            if not self.parser.has_option('core', 'user'):
                self.parser.set('core', 'user', 'skrizz')
            if not self.parser.has_option('core', 'name'):
                self.parser.set('core', 'name',
                                'Skrizz Embosbot, http://skrizz.dftba.net')
    ...
    def enumerate_modules(self, show_all=False):
        ...
        *Availability: 4.0+*
        ...
        followed by `~/.skrizz/modules`, and then through the extra directories
        ...
        # Next, look in ~/.skrizz/modules
        home_modules_dir = os.path.join(os.path.expanduser('~'), '.skrizz',
                                        'modules')
...
def wizard(section, config=None):
    dotdir = os.path.expanduser('~/.skrizz')
...
def check_dir(create=True):
    dotdir = os.path.join(os.path.expanduser('~'), '.skrizz')
    if not os.path.isdir(dotdir):
        if create:
            print 'Creating a config directory at ~/.skrizz...'
            try:
                os.makedirs(dotdir)
            except Exception, e:
                print >> sys.stderr, 'There was a problem creating %s:' % dotdir
                print >> sys.stderr, e.__class__, str(e)
                print >> sys.stderr, 'Please fix this and then run Skrizz again.'
                sys.exit(1)
        else:
            print "No config file found. Please make one before configuring these options."
            sys.exit(1)
