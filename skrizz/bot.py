# coding=utf-8
"""
bot.py - Skrizz IRC Bot
Copyright 2008, Sean B. Palmer, inamidst.com
Copyright 2012, Edward Powell, http://embolalia.net
Copyright 2012, Elad Alfassa <elad@fedoraproject.org>
Coryright 2013, Bryan Chain, http://www.bryanchain.com
...
class Skrizz(irc.Bot):
    ...
    def __init__(self, config):
        ...
        *Removed in 3.1.2*
        ...
        self.memory = self.SkrizzMemory()
        """
        A thread-safe dict for storage of runtime data to be shared between
        modules. See `SkrizzMemory <#bot.Skrizz.SkrizzMemory>`_
        """
    ...
    def setup(self):
        stderr("\nWelcome to Skrizz. Loading modules...\n\n")
        self.callables = set()
        ...
        counts = [len(modules) - 1, error_count]        

        return counts

    @staticmethod
    def is_callable(obj):
        """Return true if object is a skrizz callable.

        Object must be both be callable and have hashable. Furthermore, it must
        have either "commands" or "rule" as attributes to mark it as a skrizz
        callable.
        """
        if not callable(obj):
            # Check is to help distinguish between skrizz callables and objects
    ...
    def register(self, variables):
        """
        With the ``__dict__`` attribute from a Skrizz module, update or add the
    ...
    def unregister(self, variables):
        """Unregister all skrizz callables in variables, and their bindings.
        ...
        Args:
        variables -- A list of callable objects from a skrizz module.
        """
    ...
    class SkrizzWrapper(object):
        def __init__(self, skrizz, origin):
            self.bot = skrizz
            self.origin = origin
    ...
    class Trigger(unicode):
        def __new__(cls, text, origin, bytes, match, event, args, self):
            ...
            """
            True if the nick which triggered the command is in Skrizz's admin
    ...
    def call(self, func, origin, skrizz, trigger):
        nick = trigger.nick
            ...
            for regexp, funcs in items:
                for func in funcs:
                    ...
                    if match:
                        ...
                        skrizz = self.SkrizzWrapper(self, origin)
                        ...
                        if func.thread:
                            targs = (func, origin, skrizz, trigger)
                            t = threading.Thread(target=self.call, args=targs)
                            t.start()
                        else:
                            self.call(func, origin, skrizz, trigger)

    def debug(self, tag, text, level):
        """
        Sends an error to Skrizz's configured ``debug_target``.
        """
