# -*- coding: utf8 -*-
"""
meetbot.py - Skrizz meeting logger module
...
def configure(config):
    ...
    | --------- | ------- | ------- |
    | meeting_log_path | /home/skrizz/www/meetings | Path to meeting logs storage directory (should be an absolute path, accessible on a webserver) |
    | meeting_log_baseurl | http://example.com/~skrizz/meetings | Base URL for the meeting logs directory |
    """
...
meeting_log_path = '' #To be defined on meeting start as part of sanity checks, used by logging functions so we don't have to pass them skrizz
meeting_log_baseurl = '' #To be defined on meeting start as part of sanity checks, used by logging functions so we don't have to pass them skrizz
...
def startmeeting(skrizz, trigger):
    """
    Start a meeting.
    https://github.com/embolalia/skrizz/wiki/Using-the-meetbot-module
    """
    if ismeetingrunning(trigger.sender):
        skrizz.say('Can\'t do that, there is already a meeting in progress here!')
        return
    if not trigger.sender.startswith('#'):
        skrizz.say('Can only start meetings in channels')
        return
    if not skrizz.config.has_section('meetbot'):
        skrizz.say('Meetbot not configured, make sure meeting_log_path and meeting_log_baseurl are defined')
        return
    #Start the meeting
    ...
    meeting_log_path = skrizz.config.meetbot.meeting_log_path
    if not meeting_log_path.endswith('/'):
        meeting_log_path = meeting_log_path + '/'
    global meeting_log_baseurl
    meeting_log_baseurl = skrizz.config.meetbot.meeting_log_baseurl
    ...
    if not os.path.isdir(meeting_log_path + trigger.sender):
        try:
            ...
            skrizz.say("Can't create log directory for this channel, meeting not started!")
    ...
    skrizz.say('Meeting started! use .action, .agreed, .info, .chairs and .subject to control the meeting. to end the meeting, type .endmeeting')
...
def meetingsubject(skrizz, trigger):
    """
    Change the meeting subject.
    https://github.com/embolalia/skrizz/wiki/Using-the-meetbot-module
    """
    if not ismeetingrunning(trigger.sender):
        skrizz.say('Can\'t do that, start meeting first')
        return
    if not trigger.group(2):
        skrizz.say('what is the subject?')
        return
    if not ischair(trigger.nick, trigger.sender):
        skrizz.say('Only meeting head or chairs can do that')
        return
    ...
    skrizz.say('Current subject: ' + trigger.group(2))
meetingsubject.commands = ['subject']
meetingsubject.example = '.subject roll call'

#End the meeting
def endmeeting(skrizz, trigger):
    """
    End a meeting.
    https://github.com/embolalia/skrizz/wiki/Using-the-meetbot-module
    """
    ...
    skrizz.say("Meeting ended! total meeting length %d seconds" % meeting_length)
    ...
    skrizz.say('Meeting minutes: ' + htmllog_url)
    meetings_dict[trigger.sender] = Ddict(dict)
...
def chairs(skrizz, trigger):
    ...
    if trigger.nick.lower() == meetings_dict[trigger.sender]['head']:
        ...
        skrizz.say('Meeting chairs are: ' + chairs_readable)
    else:
        skrizz.say("Only meeting head can set chairs")

chairs.commands = ['chairs']
chairs.example = '.chairs Tyrope Jason elad'

#Log action item in the HTML log
def meetingaction(skrizz, trigger):
    """
    ...
    if not ismeetingrunning(trigger.sender):
        ...
        return
    if not trigger.group(2):
        skrizz.say('try .action someone will do something')
        return
    if not ischair(trigger.nick, trigger.sender):
        skrizz.say('Only meeting head or chairs can do that')
        return
...
def meetinginfo(skrizz, trigger):
    """
    ...
    if not ismeetingrunning(trigger.sender):
        ...
        return
    if not trigger.group(2):
        skrizz.say('try .info some informative thing')
        return
    if not ischair(trigger.nick, trigger.sender):
        skrizz.say('Only meeting head or chairs can do that')
        return
    ...
    skrizz.say('INFO: ' + trigger.group(2))
...
def log_meeting(skrizz, trigger): 
    if not ismeetingrunning(trigger.sender):
        return
