# coding=utf-8
"""
bucket.py - skrizz module to emulate the behavior of #xkcd's Bucket bot
...
To prevent skrizz from outputting a "Don't Know" message when referred use the following line:
...
from skrizz.tools import Ddict
seed()
...
def configure(config):
    ...
    information on this module at https://github.com/embolalia/skrizz/wiki/The-Bucket-Module:-User-and-Bot-Owner-Documentation
    ...
    | literal_path | /home/skrizz/www/bucket | The path in which to store output of the literal command |
    | literal_baseurl | http://example.net/~skrizz/bucket | The base URL for literal output |
    | inv_size | 15 | The maximum amount of items that Skrizz can keep. |
    """
...
class Inventory():
    ...
    def populate(self):
        ...
        while (len(self.current_items) < int(skrizz.config.bucket.inv_size)):
            self.add_random()
...
def setup(skrizz):
    ...
    rebuild_dont_know_cache(skrizz)
...
def rebuild_dont_know_cache(skrizz):
    db = connect_db(skrizz)
    cur = db.cursor()
...
def add_fact(skrizz, trigger, fact, tidbit, verb, re, protected, mood, chance, say=True):
    db = None
    cur = None
    db = connect_db(skrizz)
    cur = db.cursor()
    try:
        ...
        skrizz.say("I already had it that way!")
        return False
    finally:
        db.close()
    bucket_runtime_data.last_teach[trigger.sender] = [fact, verb, tidbit]
    if say:
        skrizz.say("Okay, " + trigger.nick)
    return True


def teach_is_are(skrizz, trigger):
    ...
    add_fact(skrizz, trigger, fact, tidbit, verb, re, protected, mood, chance)
teach_is_are.rule = ('$nick', '(.*?) (is|are) (.*)')
teach_is_are.priority = 'high'


def teach_verb(skrizz, trigger):
    ...
    if fact == tidbit and verb == '<alias>':
        skrizz.reply('You can\'t alias like this!')
        return
    say = True
    if verb == '<alias>':
        say = False
    success = add_fact(skrizz, trigger, fact, tidbit, verb, re, protected, mood, chance, say)
    if verb == '<alias>':
        db = connect_db(skrizz)
        cur = db.cursor()
        ...
        if len(results) == 0 and success:
            skrizz.say('Okay, %s. but, FYI, %s doesn\'t exist yet' % (trigger.nick, tidbit))
        if len(results) > 0 and success:
            skrizz.say('Okay, %s' % trigger.nick)
...
def save_quote(skrizz, trigger):
    ...
    try:
        ...
        skrizz.say("Sorry, I don't remember what %s said about %s" % (quotee, word))
        return
    for line in memory:
        if remove_punctuation(word.lower()) in remove_punctuation(line[0].lower()):
            ...
            if line.startswith('\001ACTION'):
                ...
                tidbit = '<%s> %s' % (quotee, line)
            result = add_fact(skrizz, trigger, fact, tidbit, verb, re, protected, mood, chance)
            if result:
                skrizz.reply("Remembered that %s <reply> %s" % (fact, tidbit))
            return
    skrizz.say("Sorry, I don't remember what %s said about %s" % (quotee, word))
...
def delete_factoid(skrizz, trigger):
    """Deletes a factoid"""
    bucket_runtime_data.inhibit_reply = trigger
    was = bucket_runtime_data.what_was_that
    if not trigger.admin:
        was[trigger.sender] = dont_know(skrizz, trigger)
        return
    db = None
    cur = None
    db = connect_db(skrizz)
    ...
    try:
        ...
        results = cur.fetchall()
        if len(results) > 1:
            skrizz.debug('bucket', 'More than one factoid with the same ID?', 'warning')
            skrizz.debug('bucket', str(results), 'warning')
            skrizz.say('More than one factoid with the same ID. I refuse to continue.')
            return
        elif len(results) == 0:
            skrizz.reply('No such factoid')
            return
        cur.execute('DELETE FROM bucket_facts WHERE ID = %s', int(trigger.group(1)))
        db.commit()
    except:
        skrizz.say("Delete failed! are you sure this is a valid factoid ID?")
        return
    finally:
        db.close()
    line = results[0]
    fact, tidbit, verb = parse_factoid(line)
    skrizz.say("Okay, %s, forgot that %s %s %s" % (trigger.nick, fact, verb, tidbit))

delete_factoid.rule = ('$nick', 'delete #(.*)')
delete_factoid.priority = 'high'


def undo_teach(skrizz, trigger):
    ...
    try:
        ...
        skrizz.say("Undo failed, this shouldn't have happened!")
        return
    finally:
        db.close()
    skrizz.say("Okay, %s. Forgot that %s %s %s" % (trigger.nick, fact, verb, tidbit))
...
def inv_give(skrizz, trigger):
    ...
    item = item.strip()
    dropped = inventory.add(item, trigger.nick, trigger.sender, skrizz)
    db = connect_db(skrizz)
    cur = db.cursor()
    search_term = ''
    if not dropped:
...
def inv_steal(skrizz, trigger):
    ...
    if (inventory.remove(item)):
        skrizz.say('Hey! Give it back, it\'s mine!')
    else:
        skrizz.say('But I don\'t have any %s' % item)
inv_steal.rule = ('^\001ACTION (steals|takes) $nickname\'s (.*)')
inv_steal.priority = 'medium'


def inv_populate(skrizz, trigger):
    bucket_runtime_data.inhibit_reply = trigger
    inventory = bucket_runtime_data.inventory
    skrizz.action('drops all his inventory and picks up random things instead')
    inventory.populate()
inv_populate.rule = ('$nick', 'you need new things(.*|)')
inv_populate.priority = 'medium'


def say_fact(skrizz, trigger):
    """Response, if needed"""
    ...
    addressed = query.lower().startswith(skrizz.nick.lower())  # Check if our nick was mentioned
    search_term = query.lower().strip()  # What we are going to pass to MySql as our search term
    if addressed:
        search_term = search_term[(len(skrizz.nick) + 1):].strip()  # Remove our nickname from the search term
    ...
    if search_term == 'shut up' and addressed:
        ...
        if trigger.sender in bucket_runtime_data.shut_up:
            ...
            skrizz.reply('I\'m back!')
        else:
            skrizz.reply('Uhm, what? I was here all the time!')
        return
    literal = False
    ...
    if search_term.startswith('literal '):
        ...
        try:
            ...
            skrizz.say('That was #%s - %s %s %s' % (factoid_id, factoid_fact, factoid_verb, factoid_tidbit))
        except KeyError:
            skrizz.say('I have no idea')
        return
    ...
    try:
        ...
        skrizz.debug('bucket', 'Warning, database encoding error', 'warning')
        skrizz.debug('bucket', e, 'warning')
    finally:
        db.close()
    if results is None:
        return
    ...
    if addressed and result is None and factoid_search is None:
        ...
        skrizz.reply('Sorry, I could\'t find anything matching your query')
        return
    elif result is None:
        return
    ...
    if literal:
        if len(results) == 1:
            ...
            number = int(result[0])
            fact, tidbit, verb = parse_factoid(result)
            skrizz.say("#%d - %s %s %s" % (number, fact, verb, tidbit))
        else:
            skrizz.reply('just a second, I\'ll make the list!')
            bucket_literal_path = skrizz.config.bucket.literal_path
            bucket_literal_baseurl = skrizz.config.bucket.literal_baseurl
            ...
            if not os.path.isdir(bucket_literal_path):
                try:
                    ...
                    skrizz.say("Can't create directory to store literal, sorry!")
                    skrizz.say(e)
                    return
            if search_term == 'random quote':
                filename = 'quotes'
            ...
            f.close()
            skrizz.reply('Here you go! %s (%d factoids)' % (bucket_literal_baseurl + web.quote(filename + '.txt'), len(results)))
        result = 'Me giving you a literal link'
    else:
        say_factoid(skrizz, fact, verb, tidbit, addressed)
...
def pick_result(results, skrizz):
    try:
        if len(results) == 1:
            result = results[0]
        elif len(results) > 1:
        ...
        skrizz.debug('bucket', 'RutimeError in pick_result', 'warning')
        skrizz.debug('bucket', e, 'warning')
        return None


def get_inventory(skrizz, trigger):
    ...
    if len(inventory.current_items) == 0:
        return skrizz.action('is carrying nothing')

    readable_item_list = ', '.join(inventory.current_items)

    skrizz.action('is carrying ' + readable_item_list)

get_inventory.rule = ('$nick', 'inventory')
get_inventory.priority = 'medium'


def connect_db(skrizz):
    return MySQLdb.connect(host=skrizz.config.bucket.db_host,
                           user=skrizz.config.bucket.db_user,
                           passwd=skrizz.config.bucket.db_pass,
                           db=skrizz.config.bucket.db_name,
                           charset="utf8",
                           use_unicode=True)
...
def dont_know(skrizz, trigger):
    ...
    try:
        ...
        rebuild_dont_know_cache(skrizz)
        return dont_know(skrizz, trigger)
...
def say_factoid(skrizz, fact, verb, tidbit, addressed):
    if verb not in bucket_runtime_data.special_verbs:
        skrizz.say("%s %s %s" % (fact, verb, tidbit))
    elif verb == '<reply>':
        skrizz.say(tidbit)
    elif verb == '<action>':
        skrizz.action(tidbit)
...
def remember(skrizz, trigger):
    ...
    try:
        ...
        return remember(skrizz, trigger)
    if len(fifo) == 10:
        fifo.pop()
