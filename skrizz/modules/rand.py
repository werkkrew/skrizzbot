...
def rand(skrizz, trigger):
    ...
    if trigger.group(2) == " " or trigger.group(2) == "" or str(trigger.group(2)) == None or str(trigger.group(2)) == "" or trigger.group(2) == None:
        skrizz.say("I'm sorry, " + str(trigger.nick) + ", but you must enter at least one number.")
    else:
        random.seed()
        ...
        if len(li_integers_str) == 1:
            ...
            skrizz.say(str(trigger.nick) + ": your random integer is: " + str(randinte))
        else:
