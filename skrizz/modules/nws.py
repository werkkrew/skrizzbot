...
def nws_lookup(skrizz, trigger):
    ...
    if len(bits) == 2:
        ...
        for line in page1:
            ...
            if mystr in line.lower():
                ...
                break
        if not url_part2:
            skrizz.reply("Could not find county.")
            return
        ...
        if bits[0]:
            ...
            if fips:
                ...
                if not state and not city:
                    skrizz.reply("Could not match ZIP code to a state")
                    return
                state = state[0].lower()
                ...
                skrizz.reply("ZIP code does not exist.")
                return

    if not master_url:
        skrizz.reply("Invalid input. Please enter a ZIP code or a county and state pairing, such as 'Franklin, Ohio'")
        return
    ...
    for item in feed.entries:
        if nomsg[:51] == item["title"]:
            skrizz.reply(nomsg.format(location))
            return
        else:
    ...
    if len(warnings_dict) > 0:
        if trigger.sender.startswith('#'):
            ...
            for key in warnings_dict:
                if i > 1: break
                skrizz.reply(key)
                skrizz.reply(warnings_dict[key][:510])
                i += 1
            skrizz.reply(more_info.format(location, master_url))
        else:
            for key in warnings_dict:
                skrizz.msg(trigger.nick, key)
                skrizz.msg(trigger.nick, warnings_dict[key])
            skrizz.msg(trigger.nick, more_info.format(location, master_url))
