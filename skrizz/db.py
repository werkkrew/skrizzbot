...
*Note:* This supersedes the ``SettingsDB`` object of 3.0. Within Skrizz modules,
...
class SkrizzDB(object):
    """
    Return a SkrizzDB object configured with the options in the given Config
    ...
    def __nonzero__(self):
        """Allow for testing if a db is set up through `if skrizz.db`."""
    ...
    def check_table(self, name, columns, key):
        """
        Return ``True`` if the SkrizzDB contains a table with the same ``name``
    ...
    def add_table(self, name, columns):
        ...
        SkrizzDB object. If a table with the same name and key already exists,
        ...
        (with the exception of other tables) of the ``SkrizzDB`` object, nor may
        ...
        query, and then creating the SkrizzDB object.
        """
...
class Table(object):
    """
    Return an object which represents a table in the given SkrizzDB, with the
